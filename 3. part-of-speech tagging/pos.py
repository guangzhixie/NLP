...
class HMM(object):
    ...
    def forward(self, sentence):
        ...
        for i, w in enumerate(sentence):
            ...
            if i == 0:
                ...
                for t in self.tagset:
                    sum_terms = [(self.emission(t, w) + self.transition(t_1, t) + alpha[(i - 1, t_1)])
                                 for t_1 in self.tagset]
                    alpha[(i, t)] = logsumexp(sum_terms)
    ...
    def build_viterbi_delta(self, sentence):
        ...
        for i, w in enumerate(sentence):
            ...
            if i == 0:
                for t in self.tagset:
                    delta[(i, t)] = self.initial(t) + self.emission(t, w)
                    
            else:
                for t in self.tagset:
                    max_term = max([(self.transition(t_1, t) + delta[(i - 1, t_1)], t_1)
                                     for t_1 in self.tagset])
                    delta[(i, t)] = self.emission(t, w) + max_term[0]
                    bp[(i, t)] = max_term[1]

...
class TestPCFG(unittest.TestCase):
    ...
    def test_pcfg(self):
        ...
        self.assertSetEqual(
                ...
                set(o.production_counts.iteritems()))
        self.assertSetEqual(set([(p.lhs(), 1) for p in tree.productions()]),
                set(o.lhs_counts.iteritems()))
        o.update_counts(tree)
        ...
        for count in o.production_counts.itervalues():
            self.assertEqual(2, count)
        self.assertEqual(3, len(o.lhs_counts))
        for count in o.lhs_counts.itervalues():
            self.assertEqual(4, count)

        o.compute_scores()
        for production, score in o.scored_productions.iteritems():
